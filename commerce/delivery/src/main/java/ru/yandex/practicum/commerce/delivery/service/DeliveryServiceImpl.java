...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    public void deliveryPicked(UUID orderId) {
        ...
        try {
            ...
            log.error("Ошибка при сборке заказа {}: {}", orderId, e.getMessage());
        }
    ...
    public Double deliveryCost(OrderDto order) {
        var address = warehouseClient.getWarehouseAddress();
        double base = 5.0;
        double multiplier = address.getStreet().contains("ADDRESS_1") ? 1.0 : 2.0;
